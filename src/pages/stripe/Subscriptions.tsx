...
const StripeSubscriptions = () => {
  ...
  const refundMutation = useMutation({
    mutationFn: (subId: string) => fetchApi(`/subscriptions/${subId}/refund`, { method: "POST" }),
    onSuccess: () => {
  ...
  return (
    <StripeLayout title="Subscriptions" subtitle="Customer Plans">
      <div className="space-y-8">
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center bg-white p-6 rounded-[2rem] border border-slate-100 shadow-xl shadow-slate-100/50">
          <div className="relative flex-1 w-full max-w-md">
            ...
            <input
              type="text"
              placeholder="Search user email or sub ID..."
              className="w-full pl-12 pr-6 py-3 rounded-2xl bg-slate-50 border-none text-sm font-medium focus:ring-2 focus:ring-indigo-500/20 transition-all"
            />
          </div>
          <button
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="border-b border-slate-50">
                  ...
                  <th className="px-8 py-6 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">History</th>
                  ...
                  >
                    <td className="px-8 py-6">
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-2xl bg-slate-100 flex items-center justify-center text-slate-500 shrink-0 border border-slate-200">
                          <User className="w-6 h-6" />
                        </div>
                        <div>
                          <p className="text-sm font-black text-slate-900 leading-none mb-1">{sub.user_name || "EverWish User"}</p>
                    ...
                    <td className="px-8 py-6">
                      <div className="flex flex-col gap-2">
                        <div className="flex items-center gap-2">
                          <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest bg-${getStatusColor(sub.status)}-500/10 text-${getStatusColor(sub.status)}-600 border border-${getStatusColor(sub.status)}-500/20`}>
                            {sub.status}
                          </span>
                        ...
                        <p className="text-[10px] font-bold text-slate-400 flex items-center gap-1 uppercase tracking-tighter italic">
                      ...
                      <div className="space-y-1">
                        ...
                        {sub.cancel_at_period_end && (
                          <div className="flex items-center gap-2 px-2 py-1 rounded-lg bg-rose-50 border border-rose-100 w-fit">
                            ...
                            <span className="text-[9px] font-black text-rose-600 uppercase tracking-tight">Pending Cancellation</span>
                          </div>
                        )}
                      </div>
                    </td>
                    <td className="px-8 py-6">
                      <div className="flex items-center gap-4 text-slate-400">
                        <div className="text-center">
                          <p className="text-[10px] font-black uppercase tracking-tighter leading-none mb-1">Interval</p>
                          <p className="text-sm font-black text-slate-700 capitalize leading-none">{sub.billing_interval}</p>
                        </div>
                        <div className="w-px h-8 bg-slate-100" />
                        <div className="text-center">
                          <p className="text-[10px] font-black uppercase tracking-tighter leading-none mb-1">Payment</p>
                          <p className={`text-sm font-black leading-none ${sub.last_payment_status === 'paid' ? 'text-emerald-600' : 'text-rose-500'}`}>
                            {sub.last_payment_status === 'paid' ? 'Success' : 'Failed'}
                          </p>
                        </div>
                      </div>
                    </td>
        ...
        <div className="flex justify-center">
          <div className="flex gap-2 p-2 rounded-2xl bg-white border border-slate-100">
            <button disabled={page === 1} onClick={() => setPage(p => p - 1)} className="px-4 py-2 rounded-xl border border-slate-100 text-xs font-bold hover:bg-slate-50 disabled:opacity-50">Prev</button>
            <div className="px-4 py-2 rounded-xl bg-slate-900 text-white text-xs font-black">{page}</div>
            <button disabled={!data?.pagination?.hasNext} onClick={() => setPage(p => p + 1)} className="px-4 py-2 rounded-xl border border-slate-100 text-xs font-bold hover:bg-slate-50 disabled:opacity-50">Next</button>
          </div>
        </div>
      </div>
    </StripeLayout>
